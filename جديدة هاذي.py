...
bot = telebot.TeleBot("7445982313:AAHq9U8dcaMDLMe1F2dJntNmxr5U2THxxuY")
...
def get(message):
    ...
    r1 = requests.get("https://github.com/Abdoumihoubi/Abdouhcrozon/blob/main/fddfvvffgf/hhhhhh").text
    if '05' in mobile_number and str(user_id) not in  str(r1):    	
    	tu = types.InlineKeyboardButton('المطور',url='@abdoumihou2000')
...
def otp(message):
    ...
    if access_token:
    	m = 0
    	count_reference = 0
    	bot.send_message(chat_id=message.chat.id,text='تــــــــم تحقق بنجاح من رمز إنتضــــــر قليـلا نواصلو عملــــــــية تسجيـل🥀💗')
    ...
    'User-Agent': 'okhttp/4.9.3',
       ...
       "mgmValue":mgm
    	...
    	res1= response
    	if 'مرجع' in res1:
    		 bot.send_message(chat_id=message.chat.id,text='يتــــــــم لإرسااااال ضرك يڨلك شحال جاتك متقلقش برك 🙂♾️')
    	bitch = get_balance(access_token)
    	for account in bitch['accounts']:
    	    if account['label'] == 'Bonus parrainage':
    	        bot.send_message(chat_id=message.chat.id, text=f"""نجحتـ عمليــة لإرســــال مبـــــروك 🎉⬇️ {count_reference}GO
    		      	<strong>your bonus now: {account['value']}
    		      	 by @abdoumihou2000</strong>""",parse_mode='html')
    	else:
    		 pass
    else:
